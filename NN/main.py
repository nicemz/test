import numpy as np       #处理矩阵运算的库
import scipy.special     #科学运算的库
...
class neuralNetwork :
    ...
    def __init__(self, inputnodes, hiddennodes, outputnodes, learningrate):
        ...
        # 初始化输入层与隐层之间的权重
        #np.random.normal用于产生正态分布数据，这里以μ=0.0，标准差为隐藏层节点数开方分之一，生成随机数的个数为输入层节点数乘以输出层节点数
        #注意：正态曲线的拐点对应的横坐标-μ=标准差，，而公式中的取值范围正好可以对应到标准差，所以直接以此参数为标准差输入
    ...
    def test(self, inputs_list):
        ...
        #假设inputs_list的长度为len，通过np.array(inputs_list, ndmin=2)这条语句会被复制并转换为一个大小为(1,len)的矩阵。
        # 转置以后便是一个大小为(len,1)的矩阵(.T表示将矩阵转置)
        inputs = np.array(inputs_list, ndmin=2).T
...
def pre_one():
    ...
    # 打印测试数据标签
    test_data = test_data_list[3].split(',')
